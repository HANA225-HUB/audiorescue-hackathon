...
class AudioLevelMetrics:
    peak_abs: float
    rms_dbfs: float | None                  # 数字静音为 None
...
class ProcessResult:
    ...
    mixed_output_path: str | None
    original_levels: AudioLevelMetrics | None
    mixed_levels: AudioLevelMetrics | None
...
```json
{
  ...
  "full_output_path": "outputs/.../enhanced_full.wav",
  "mixed_output_path": "outputs/.../enhanced_mix.wav",
  "original_levels": {"peak_abs": 0.91, "rms_dbfs": -21.6},
  "mixed_levels": {"peak_abs": 0.86, "rms_dbfs": -22.1},
  "transcript_before": {
...
- pipeline 必须从冻结配置显式传 `model_name` 与解析后的 `device`，不得在转写函数内静默换模型或设备；
...
| 真实样例 | `real_spkA_traffic_r01.wav` | 真实场景展示；无严格 clean 对齐指标 |
