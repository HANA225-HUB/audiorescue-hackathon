...
最终现场链路以 [`../OFFLINE_DEMO_RUNBOOK.md`](../OFFLINE_DEMO_RUNBOOK.md) 为准。本手册中的服务器、租卡和旧机器记录只作为背景，不是通用启动命令。公开文档不得写入服务器地址、凭据、个人邮箱、本机绝对路径、私有录音名、私有参考文本、模型缓存位置或原始日志。

当前规则：

- Python 使用 3.10 或 3.11；
- PyTorch 与 TorchAudio 必须同版本、同 CPU/CUDA 来源；
- 生产入口关闭 fixture，只调用 `process_audio`；
- fixture 只验证 UI 状态和离线页面，不代表真实增强或转写效果；
- 真实 smoke 只在已授权短样例和已预热模型上运行；
- smoke CLI 只公开安全 JSON 摘要，正常 stderr 应为空；
- 现场清理只删除本轮临时 outputs/staging 和服务进程，不删除已验证环境或模型缓存。
...
```text
默认 Python：可能不是项目目标版本
项目 Python：3.10 或 3.11
FFmpeg：必须在当前 shell 可用
```

因此不要在不兼容的系统 Python 中安装项目包。Whisper 官方仓库给出的预期兼容范围覆盖 Python 3.10/3.11；本项目使用独立虚拟环境。FFmpeg 是必须先确认的系统依赖。
...
若换到另一台演示机且没有 Python 3.10/3.11，先按该机器的包管理方式安装目标 Python。安装后确认：
...
预期 `python --version` 为 3.10.x 或 3.11.x，当前 shell 的 `python` 来自项目虚拟环境。
...
```bash
rsync -av <remote-precomputed-assets> ./demo_assets/precomputed/
rsync -av <remote-safe-log-summaries> ./logs/server/
```

实际执行前替换远端占位符。先用少量文件验证方向，避免把空目录覆盖到有内容的目录。
...
```bash
ssh -L 7860:localhost:7860 <remote-host>
```

浏览器打开本地 Gradio 页面。不要为了方便把 Gradio 无鉴权暴露到公网；更不要上传真实会议或采访隐私录音到开放服务。
